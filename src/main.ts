...
function applyBodyExtinctionAndPhase(
  ...
  for (const entry of solarSystem.getAllBodies()) {
    ...
    if (id === 'venus' || id === 'mercury') {
      ...
      const obsFromBody = dir.clone().multiplyScalar(-1); // dir is body→cam already, so obs from body = cam−body normalised
      // Recompute properly: obs from body = (cam − body), but `dir` is normalised (body − cam) reversed... actually `dir` was body − cam. So obs from body is −dir.
 ...
 */
function applyAtmosphericFlattening(cameraPos: Vector3, zenith: Vector3, sunWorld: Vector3): void {
  void sunWorld;
...
function updateSkyForObserver(): void {
  ...
  if (cameraCtl.getMode() !== 'observer') {
    ...
    const moonReset = solarSystem.getBody('moon');
    if (moonReset) moonReset.mesh.setEarthshine(0, _earthshineSunDir);
    return;
  }
  ...
  if (solarSystem.realism.get().extinction) {
    applyRefractionLift(ctx.cameraPos, ctx.zenith);
  }
  ...
  applyBodyExtinctionAndPhase(realismState.extinction, ctx.cameraPos, ctx.zenith, sunWorld);

  // Earthshine on moon — peaks at new crescent (Earth full from moon).
  if (realismState.moonGlow) {
    applyEarthshine(ctx.cameraPos, sunWorld);
  } else {
  ...
  applyAtmosphericFlattening(ctx.cameraPos, ctx.zenith, sunWorld);
