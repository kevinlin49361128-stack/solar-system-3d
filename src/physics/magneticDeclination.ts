 ...
 *
 * Model: a truncated WMM 2025 spherical-harmonic expansion to order n=2
 * (dipole + quadrupole). Uses 13 Gauss coefficients total:
 *
 *   n=1 (dipole):     g₁⁰, g₁¹, h₁¹                              — 3
 *   n=2 (quadrupole): g₂⁰, g₂¹, h₂¹, g₂², h₂²                    — 5
 *   (m=0 terms have no h component, so the totals are 3 and 5.)
 *
 * Versus the full 12th-order WMM (168 coefficients), this catches the
 * global declination pattern to ~3° in most populated regions. Hotspots
 * with strong local crustal anomalies (Hudson Bay, southern Africa)
 * still take an additional 5° hit; acceptable for an AR sky aid given
 * the phone gyro's own 1–3° noise floor.
 *
